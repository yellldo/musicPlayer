...
public class UserInfoBiz {
    ...
    public UserInfoVo selectByPhone(String phone) {
        UserInfo userInfo = userInfoService.selectOne(UserInfo::getPhone, phone, UserInfo::getIsDelete, CommonConstants.STATUS_NOT_DEL);
        if (BeanUtil.isEmpty(userInfo)) {
            return new UserInfoVo();
        }
        return new UserInfoVo().setUserId(userInfo.getUserId()).setNickName(userInfo.getNickName()).setUserStatus(userInfo.getUserStatus());
    }

    public void create(CreateUserInfoDto createUserInfoDto) {
        UserInfo userInfo = UserInfoConvert.INSTANT.create(createUserInfoDto);
        userInfo.setIsDelete(CommonConstants.STATUS_NOT_DEL);
        userInfo.setUserStatus(UserConstants.USER_STATUS_NORMAL);
        userInfo.setUserType(UserConstants.USER_TYPE_NORMAL);
