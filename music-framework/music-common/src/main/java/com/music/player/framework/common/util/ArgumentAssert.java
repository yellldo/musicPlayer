...
import com.music.player.framework.common.support.ArgumentException;
...
/**
 * ClassName : ArgumentAssert<br>
 * Description : 断言某些对象或值是否符合规定，否则抛出异常。经常用于做变量检查<br>
 *
 * @author : sj
 * @date : 2024/8/29
 */
public class ArgumentAssert {
    ...
    public static int checkIndex(int index, int size)
            throws ArgumentException, IndexOutOfBoundsException {
    ...
    public static int checkBetween(int value, int min, int max) {
        if (value < min || value > max) {
            throw new ArgumentException(
                    StrUtil.format("Length must be between {} and {}.", min, max));
        }
        return value;
    }
    ...
    private static String badIndexMsg(int index, int size, String desc, Object... params) {
        if (index < 0) {
            return StrUtil.format("{} ({}) must not be negative", StrUtil.format(desc, params),
                    index);
        } else if (size < 0) {
            throw new ArgumentException("negative size: " + size);
        } else { // index >= size
            return StrUtil.format("{} ({}) must be less than size ({})",
                    StrUtil.format(desc, params), index, size);
        }
    }
}
