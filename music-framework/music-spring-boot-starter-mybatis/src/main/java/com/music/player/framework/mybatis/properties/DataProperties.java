...
import com.music.player.framework.common.constant.enums.IdTypeEnum;
import com.music.player.framework.common.constant.enums.MultiTenantType;
...
import static com.music.player.framework.mybatis.properties.DataProperties.DATA_PREFIX;

/**
 * ClassName : DataProperties<br>
 * Description : CustomMybatisProperties配置属性类<br>
 *
 * @author : sj
 * @date : 2024/5/1
 */
...
public class DataProperties {
    ...
    /**
     * 参数说明参考：https://github.com/baidu/uid-generator 长期运行：重启频率为12次/天, 那么配置成{"workerBits":23,"timeBits":31,"seqBits":9}时, 可支持28个节点以整体并发量14400
     * UID/s的速度持续运行68年 长期运行+集群数量少+并发稍高：{"workerBits":20,"timeBits":31,"seqBits":12}这样的配置可以68年，100W次重启，单机每秒4096个并发的情况 频繁重启：重启频率为24*12次/天,
    ...
    public static class CacheId extends DefaultId {
        ...
        /**
         * 指定何时向RingBuffer中填充UID, 取值为百分比(0, 100), 默认为50 举例: bufferSize=1024, paddingFactor=50 -> threshold=1024 * 50 / 100 = 512. 当环上可用UID数量
         * < 512时, 将自动对RingBuffer进行填充补全
         */
        private int paddingFactor = RingBuffer.DEFAULT_PADDING_PERCENT;
        /**
         * 另外一种RingBuffer填充时机, 在Schedule线程中, 周期性检查填充 默认:不配置此项, 即不实用Schedule线程. 如需使用, 请指定Schedule线程时间间隔, 单位:秒
         */
        ...
        /**
         * 拒绝策略: 当环已空, 无法继续获取时 默认无需指定, 将记录日志, 并抛出UidGenerateException 异常. 如有特殊需求, 请实现RejectedTakeBufferHandler接口(支持Lambda表达式)
         */
    ...
    public static class DefaultId {
        ...
        /**
         * 当前时间，相对于时间基点"${epochStr}"的增量值，单位：秒， 28: 大概可以使用 8.7年, 28位即最大表示2^28的数值的秒数 30: 大概可以使用 34年, 30位即最大表示2^30的数值的秒数 31: 大概可以使用 68年,
         * 31位即最大表示2^31的数值的秒数
         */
         ...
         * 机器id，
         * <p>
         * 20：100W次重启 22: 最多可支持约420w次机器启动。内置实现为在启动时由数据库分配。420w = 2^22 23：800w次重启  12次/天 27: 1.3亿次重启 24*12次/天
         */
        private int workerBits = 23;
        /**
         * 每秒下的并发序列，13 bits可支持每秒8192个并发，即2^13个并发 9: 512 并发 13: 8192 并发
         */
